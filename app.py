...
# =========================================================
# SECTION 4 : 📉 CHUTE DES COTES (VERSION CORRIGÉE & ULTRA CLAIRE)
# =========================================================
    ...
    if cle_chute == "DADY2026":
            ...
            try:
                if "sofascore.com/" in lien_lower and "/match/" in lien_lower:
                    ...
                    if "/h2h/" in lien_lower:
                        ...
                        # Structure Oddsportal H2H : domicile / exterieur
        ...
        # --- GÉNÉRATEUR MATRICIEL COHÉRENT (Pas de contradictions) ---
        # 1. Sélection du Score Exact en premier pour dicter la logique des autres marchés
        scores_liste = ["1 - 0", "2 - 0", "2 - 1", "0 - 1", "0 - 2", "1 - 2", "1 - 1", "0 - 0", "2 - 2", "3 - 1"]
        option_score = scores_liste[seed % len(scores_liste)]
        
        # Extraction des buts du score pour éliminer les bugs de logique
        buts_dom = int(option_score.split(" - ")[0])
        buts_ext = int(option_score.split(" - ")[1])
        total_buts = buts_dom + buts_ext
        
        # 2. Alignement automatique et logique du Marché Principal (Under/Over)
        if total_buts >= 3:
        ...
        # 5. Options Mi-temps isolées et Seconde période (1X2)
        ...
        # 6. Cotes d'ouverture et baisse réaliste
        ...
        # =========================================================
        # 📊 AFFICHAGE VISUEL PREMIUM ET ULTRA DÉTAILLÉ
        # =========================================================
        ...
        with col_gauche:
            ...
            st.info(
                f"• **Marché Principal :** `{option_jeu}`\n\n"
                f"• **Les 2 Équipes Marquent :** `{option_btts}`\n\n"
        ...
        with col_droite:
            ...
            st.warning(
                f"• **Résultat Mi-temps (1X2) :** `{option_ht}`\n\n"
                f"• **Scénario Complet HT/FT :** `{option_ht_ft}`\n\n"
                f"• **Deuxième Période (1X2) :** `{option_p2}`"
            )

        # Indicateur de Chute de cote
        st.markdown("### 📉 Analyse des Flux Financiers (Chute de Cote)")
        st.error(
            f"⚽ **Mouvement du marché mondial :**\n\n"
            f"• Cote d'Ouverture : `{cote_ouvrir}` ➔ Cote Actuelle : `{cote_actu}`\n\n"
            f"📉 Intensité de la baisse enregistrée : **-{baisse:.2f}%**"
        )
        
        st.markdown("---")
        st.subheader("🎯 Conseil de Validation Automatique")
        st.success(f"🔥 **ANALYSE VALIDÉE POUR LE MATCH : {nom_du_match}**\n\n"
                   f"L'indice de confiance montre une forte injection de capitaux sur le marché `{option_jeu}` combiné avec un pronostic de score exact `{option_score}`. Recommandation : Suivre la tendance avec une mise modérée.")
