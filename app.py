...
if st.button("🚀 ANALYSER LES VRAIS MATCHS", type="primary", use_container_width=True):
    try:
        ...
        matches = [m for m in matches if m.get("status") not in ["FINISHED", "CANCELLED", "POSTPONED"]]

        if not matches:
            st.error("❌ Aucun match à venir trouvé pour cette date dans ces compétitions.")
            st.stop()
        ...
        if not results:
            st.error("❌ Impossible de calculer les probabilités (pas assez d'historique disponible).")
            st.stop()
