# -*- coding: utf-8 -*-
"""
RODRIGUE PRO FOOTBALL AI — V3 DYNAMIC + BLUE SCORE
===================================================

Application Streamlit d'analyse football multi-facteurs avec Dashboard Blue Score.
"""
...
def data_quality(
    ...
    if home_stats["home_gf"] is not None:
        score += 1
    if away_stats["home_ga"] is not None:
        score += 1
    if home_table:
        score += 1
    if away_table:
...
if report:
    ...
    # ============================================================
    # DASHBOARD BLUE SCORE (STYLE VISUEL UNIQUE)
    # ============================================================
    h_name = report["home"]
    a_name = report["away"]
    p1 = report["markets"]["1"]
    px = report["markets"]["X"]
    p2 = report["markets"]["2"]
    btts_val = report["markets"]["BTTS Oui"]
        ...
        <div style="background: linear-gradient(135deg, #0b1d3a, #163b6d); padding: 20px; border-radius: 12px; color: white; text-align: center; box-shadow: 0 4px 15px rgba(0,0,0,0.3); margin-top: 20px;">
            ...
            <div style="display: flex; justify-content: space-around; background: rgba(0,0,0,0.3); padding: 12px; border-radius: 8px; margin-top: 15px;">
                <div style="text-align: center;">
                    ...
                    <strong style="font-size: 16px; color: #4ade80;">{p1:.2f}%</strong>
                </div>
                <div style="text-align: center;">
                    <span style="display: block; font-size: 11px; color: #94a3b8;">X (NUL)</span>
                    <strong style="font-size: 16px; color: #facc15;">{px:.2f}%</strong>
                </div>
                <div style="text-align: center;">
                    <span style="display: block; font-size: 11px; color: #94a3b8;">2 (EXT)</span>
                    <strong style="font-size: 16px; color: #f87171;">{p2:.2f}%</strong>
                </div>
            </div>
            ...
            <div style="margin-top: 12px; background: rgba(15, 23, 42, 0.7); padding: 10px; border-radius: 8px; text-align: center;">
                ...
                <strong style="font-size: 15px; color: #fff;">{btts_val:.1f}%</strong>
            </div>
        </div>
        """,
    ...
    # Top 3 Scores Recommandés en blocs visuels
    st.markdown("#### 🎯 Top Scores Recommandés (Blue Score)")
    top_scores = report['exact_scores'][:3]
