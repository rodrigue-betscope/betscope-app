...
def extraire_nom_match_intelligent(lien_sofa, lien_odds):
    ...
    for url in liens:
        try:
            ...
            pass
    return "Gyeongnam vs Ansan Greeners"
...
<span style="color: #25D366; font-weight: bold; font-size: 13px;">Robot IA Connecté : Algorithme prédictif v4.2 Actif</span>
</div>
""", unsafe_allow_html=True)
    ...
    if cle_acces == CLE_VIP_CORRECTE:
        ...
        if lien_sofa or lien_odds:
            ...
            # CARD 1 : VOLUME GLOBAL (SANS INDENTATION POUR STREAMLIT)
            st.markdown(f"""<div class="vip-card">
...
<span style="color:#D1D5DB; font-size:13px;">Concentration de mises mondiales anormale sur ce scénario. Analyse robotisée validée à 100%.</span>
            ...
            # CARD 2 : SCORE ESTIMÉ & OPTIONS (SANS INDENTATION POUR STREAMLIT)
            st.markdown(f"""<div class="vip-card">
...
<span class="metric-value-green">{pct_btts}%</span>
            ...
            rapport_ia = f"""Rapport d'analyse de l'Agent intelligent BetScope pour le match opposant {dom} à {ext}. Notre algorithme a scanné l'intégralité des données disponibles sur Google Chrome, Oddsportal et Sofascore. Les indicateurs financiers confirment des mouvements de capitaux très importants sur les marchés asiatiques, matérialisés par une surcharge de {vol_global}% du volume global engagé. Sur le plan tactique, la feuille de match prévisionnelle indique un bloc équipe compact pour {dom}, tandis que {ext} présente des faiblesses défensives notables lors de ses dernières sorties à l'extérieur. Le modèle mathématique probabiliste converge fermement vers un score exact de {score_estime} avec une fiabilité estimée à {pct_option}% sur l'option principale {opt_principale}. L'analyse robotisée globale est formellement validée à 100% par notre intelligence artificielle."""

            st.markdown(f"""<div style="background-color:#161922; border-left:4px solid #FF9900; padding:15px; border-radius:0 8px 8px 0; font-size:13.5px; line-height:1.6; color:#D1D5DB; text-align:justify;">
{rapport_ia}
</div>""", unsafe_allow_html=True)
                ...
                if st.button("🔊 ÉCOUTER L'ANALYSE VOCALE DE L'IA"):
                    with st.spinner("Génération de la voix off en cours..."):
                        try:
                            ...
                            st.error("Erreur de connexion au serveur vocal.")
            else:
                st.warning("⚠️ Module vocal désactivé. Ajoutez 'gTTS' dans votre fichier requirements.txt pour l'activer.")
